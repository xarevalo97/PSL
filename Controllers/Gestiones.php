...
class Gestiones extends Controllers{
	...
	public function Gestiones()
	{
		...
		$data['page_tag'] = "Gestiones";
		$data['page_title'] = "GESTIONES <small></small>";
		$data['page_name'] = "gestiones";
		$data['page_functions_js'] = "functions_gestiones.js";
		$this->views->getView($this,"gestiones",$data);
	}

	public function setGestiones(){
		error_reporting(0);
		if($_POST){
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['txtNit']) || empty($_POST['txtNombreFiscal']) || empty($_POST['txtDirFiscal']) )
			{
				...
				if($idUsuario == 0)
				{
					...
					if($_SESSION['permisosMod']['w']){
						$request_user = $this->model->insertGestiones($strIdentificacion,
																			$strNombre, 
																			$strApellido, 
																			$intTelefono, 
																			$strEmail,
					...
					if($_SESSION['permisosMod']['u']){
						$request_user = $this->model->updateGestiones($idUsuario,
																	$strIdentificacion, 
																	$strNombre,
																	$strApellido, 
																	$intTelefono, 
	...
	public function getGestiones()
	{
		if($_SESSION['permisosMod']['r']){
			$arrData = $this->model->selectTarjetas();
	...
	public function getGestion($idpersona){
		if($_SESSION['permisosMod']['r']){
			$idusuario = intval($idpersona);
			if($idusuario > 0)
			{
				$arrData = $this->model->selectTarjeta($idusuario);
				if(empty($arrData))
				{
	...
	public function delGestion()
	{
		if($_POST){
			if($_SESSION['permisosMod']['d']){
				$intIdpersona = intval($_POST['idUsuario']);
				$requestDelete = $this->model->deleteGestion($intIdpersona);
				if($requestDelete)
				{
