...
class Creditos extends Controllers{
			...
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['txtNit']) || empty($_POST['txtNombreFiscal']) || empty($_POST['txtDirFiscal']) )
			{
				...
				if($idUsuario == 0)
				{
					...
					if($_SESSION['permisosMod']['w']){
						$request_user = $this->model->insertCreditos($strIdentificacion,
																			$strNombre, 
																			$strApellido, 
																			$intTelefono, 
																			$strEmail,
					...
					if($_SESSION['permisosMod']['u']){
						$request_user = $this->model->updateCreditos($idUsuario,
																	$strIdentificacion, 
																	$strNombre,
																	$strApellido, 
																	$intTelefono, 
	...
	public function getCredito($idpersona){
		if($_SESSION['permisosMod']['r']){
			$idusuario = intval($idpersona);
			if($idusuario > 0)
			{
				$arrData = $this->model->selectCreditos($idusuario);
				if(empty($arrData))
				{
	...
	public function delCredito()
	{
		if($_POST){
			if($_SESSION['permisosMod']['d']){
				...
				$requestDelete = $this->model->deleteCreditos($intIdpersona);
				if($requestDelete)
				{
