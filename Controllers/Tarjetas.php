...
class Tarjetas extends Controllers{
	...
	public function Tarjetas()
	{
		...
		$data['page_tag'] = "Tarjetas";
		$data['page_title'] = "TARJETAS <small></small>";
		$data['page_name'] = "tarjetas";
		$data['page_functions_js'] = "functions_tarjetas.js";
		$this->views->getView($this,"tarjetas",$data);
	}
			...
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['txtNit']) || empty($_POST['txtNombreFiscal']) || empty($_POST['txtDirFiscal']) )
			{
				...
				if($idUsuario == 0)
				{
					...
					if($_SESSION['permisosMod']['w']){
						$request_user = $this->model->insertTarjeta($strIdentificacion,
																			$strNombre, 
																			$strApellido, 
																			$intTelefono, 
																			$strEmail,
					...
					if($_SESSION['permisosMod']['u']){
						$request_user = $this->model->updateTarjeta($idUsuario,
																	$strIdentificacion, 
																	$strNombre,
																	$strApellido, 
																	$intTelefono, 
	...
	public function getTarjeta($idpersona){
		if($_SESSION['permisosMod']['r']){
	...
	public function delTarjeta()
	{
		if($_POST){
			if($_SESSION['permisosMod']['d']){
				...
				$requestDelete = $this->model->deleteTarjeta($intIdpersona);
				if($requestDelete)
				{
